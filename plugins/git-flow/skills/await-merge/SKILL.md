...
2. 없으면 현재 브랜치의 open PR 1건을 `<가이드: 현재 브랜치의 열린 PR 번호>` 로 자동 검색:
...
`<가이드: PR 정보 조회>` 로 정규화된 JSON `{number, state, headRefName, baseRefName, mergedAt, url}` 을 얻는다 (`PR_INFO`). 가이드 명령이 host 차이를 흡수해 `state` 는 항상 `OPEN`/`MERGED`/`CLOSED` 셋 중 하나 (GitLab `opened` → `OPEN` 정규화).
...
`Monitor` 도구를 `persistent: true` 로 무장. 60s 간격, 사용자 요청 짧을 시 30s 까지 허용. 루프 안의 정보 조회는 `<가이드: PR 정보 조회>` 명령을 사용:

```bash
PR_NUM=<번호>
INTERVAL=${INTERVAL:-60}
while true; do
  info="$( <가이드: PR 정보 조회> )"   # 정규화 JSON, 실패 시 stderr 포함 캡처
...
1. **base 브랜치 결정 — orch settings 의 프로젝트별 값 우선**:
   ...
   - **PR.baseRefName 사전 검증**: §2 에서 얻은 `baseRefName` 값이 `BASE_BRANCH` 와 다르면 (예: release 브랜치로 머지) 경고하고 cleanup 보류 — 자동 switch 시 사용자가 기대한 branch 와 어긋남.
2. 워킹 트리 깨끗한지 확인:
   ```bash
...
- ❌ 플랫폼 분기를 SKILL 본문에 인라인 — 분기가 흩어져 한쪽 host 만 동작
  ✅ §0 에서 1회 탐지 후 GITHUB_GUIDE.md / GITLAB_GUIDE.md 의 명령만 사용 (정규화된 state 사용)
- ❌ base 브랜치 하드코딩 (`develop` / `main`) 또는 글로벌 단일값 의존 — 프로젝트마다 base 가 다른데 한 값으로 고정 → switch 실패로 cleanup 전체 중단 + pull 누락
  ✅ orch settings.json 의 `projects.<alias>.default_base_branch` (프로젝트별, validate-harness hook 이 필수 보장), orch 외부 환경에서만 `<가이드: 호스트 기본 브랜치>` fallback
...
- ❌ PR baseRefName 검증 생략 — PR 이 release/staging 같은 비기본 브랜치로 머지된 경우에도 default 로 switch 해버려 mismatch
  ✅ `<가이드: PR 정보 조회>` 의 `baseRefName` 으로 PR 실제 base 와 결정한 BASE_BRANCH 비교, 다르면 사용자 확인
...
Claude: → §0 탐지 (GF_HOST=gitlab) → GITLAB_GUIDE.md
        → <가이드: PR 정보 조회>: state=OPEN, headRefName=feat/x
        → 폴링 ... → state=MERGED
