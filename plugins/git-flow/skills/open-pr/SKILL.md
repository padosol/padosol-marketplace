...
#### 이슈 키 추출 (선택 — 없으면 그냥 생략)

**이슈 키는 필수가 아니다.** 아래 순서로 찾아보고, 못 찾으면 **사용자에게 묻지 말고 조용히 생략**한 채 진행한다. 이슈 트래커를 쓰지 않는 저장소가 정상 케이스다.

1. 현재 브랜치명 정규식:
   - Linear 형: `^(feature|fix|refactor|chore|docs|hotfix)/(MP-\d+)-` → `MP-<번호>`
   - GitHub Issues 형: `^(feature|fix|refactor|chore|docs|hotfix)/(\d+)-` → `#<번호>`
2. 실패 시 최근 커밋 메시지: `git log -1 --format=%s | grep -oE '(MP-[0-9]+|#[0-9]+)'`
3. 그래도 없으면 → **`ISSUE_KEY` 를 빈 값으로 두고 진행.** 질문 금지, placeholder(`MP-XXX`, `#0`) 생성 금지.

placeholder 를 만들어 넣으면 매직워드가 존재하지 않는 이슈를 가리켜 자동 전이가 엉키고, 제목에도 의미 없는 문자열이 남는다. **모르면 비우는 쪽이 항상 낫다.**

사용자가 `/open-pr MP-42` 또는 `/open-pr #42` 처럼 인자로 키를 직접 준 경우 그것을 최우선으로 쓴다.

#### PR 제목

- 이슈 키 없음 (기본): `<type>: <한글 요약>`
- 이슈 키 있음: `<type>: <이슈키> <한글 요약>` (예: `fix: MP-80 라우팅 tier 보존`)

규칙:
- type 은 브랜치 prefix 와 동일 (`feature` → `feat`, `hotfix` → `fix` 등 매핑)
- 한글 요약은 머지 대상 커밋들의 첫 줄 분석으로 도출. 단일 커밋이면 그 메시지 본문 차용. 다중이면 가장 큰 변경 덩어리 기준.
- 커밋 컨벤션(`<type>: <이슈키> <설명>`)과 같은 모양을 유지한다. 대괄호 접두(`[MP-42] feat: …`)는 쓰지 않는다.
...
본문은 아래 순서로 조립하고, 푸터는 변수 조립 후 **별도 단계로 강제 append** 한다. 본문을 자유 형식으로 즉석 작성하면 컨텍스트 누적 시 Claude 기본 푸터로 회귀해 PR 컨벤션이 깨진다. 본문 생성 파이프라인은 "**프로젝트 템플릿 탐지 → (프로젝트 템플릿 채우기 | 호스트 가이드가 지정한 내장 fallback 템플릿 채우기) → footer append → 검증 (다음 단계)**" 결정적 4단계.

##### 1) 프로젝트 템플릿 탐지

프로젝트에 PR/MR 템플릿이 있으면 **반드시 그 템플릿을 본문 토대로 사용**한다. 템플릿이 있는데 내장 골격으로 대체하는 것 금지.

`<가이드: PR 템플릿 위치>` 섹션의 경로 우선순위대로 템플릿 파일을 찾아 `TEMPLATE_BODY` 로 로드한다. (GitHub 디렉토리형 멀티 템플릿이 여러 개면 자동 선택하지 말고 목록을 사용자에게 보여 주고 고르게 한 뒤 그 파일을 로드.) 못 찾으면 아래 3) 내장 fallback 템플릿.
...
- **`ISSUE_KEY` 가 있을 때만** 이슈를 연결한다: 템플릿에 이슈/티켓 슬롯이 있으면 거기에, 없으면 본문 마지막(푸터 직전 줄)에 `Closes <이슈키>` 를 append. `ISSUE_KEY` 가 비어 있으면 **연결 줄 자체를 넣지 않고**, 템플릿에 이슈 슬롯이나 안내 주석이 있으면 그 줄을 삭제한다.
- 채울 근거가 없는 섹션은 비워 두지 말고 `N/A` 또는 한 줄 사유를 적는다 (빈 헤더 방지).

##### 3) 프로젝트 템플릿이 없을 때 — 내장 fallback 템플릿

프로젝트 템플릿을 못 찾았을 때, `<가이드: 내장 fallback 템플릿>` 이 지정하는 이 스킬 디렉토리 `templates/` 의 파일을 `TEMPLATE_BODY` 로 로드한 뒤 **2) 와 동일한 채우기 로직**으로 슬롯만 채운다 (섹션 헤더·순서·체크리스트 항목 보존, 안내 주석 `<!-- -->`·빈 bullet 자리 채운 뒤 주석 제거, 근거 없는 섹션은 `N/A` 또는 생략).

어떤 파일을 쓰는지와 상황 판단 여부는 **호스트 가이드가 정한다** — 여기서 판단 로직을 재구현하지 말고 가이드의 해당 섹션을 그대로 따른다. (참고용 요약: GitHub 은 변경 성격 4종 자동 판단, GitLab 은 단일 `templates/mr.md` 고정 + 템플릿의 `## 변경 유형` 체크박스 택1.)

슬롯 채우기 공통 규칙 (내장 템플릿에도 적용):
- **요약**: 정확히 1~3 bullet. 4개 이상이면 가장 큰 변경 1~3 으로 압축
- **변경 내용**: 각 bullet 은 `` `<path>` — <한 줄 설명> `` 형태. 변경 파일이 1개거나 docs/config 만 변경 시 섹션을 통째로 생략 (빈 헤더 금지)
- **확인 / 동작 동일성 / 재현 등 체크리스트**: 단순 변경 1~2 항목, 복합 변경 (배포·DB 마이그레이션·다중 컴포넌트) 4~6 항목. 각 항목은 검증 주체가 명확해야 함 (예: `bash -n 통과`, `EC2 cutover 후 health check 200`). **실제로 수행한 검증만** 체크 표시하고, 남이 해야 할 확인은 빈 체크박스로 둔다
- **규약 체크리스트 (`## 체크리스트` 등)**: 항목을 지우거나 새로 만들지 않는다. 스킬이 **실제로 확인한 항목만** `- [x]`, 확인 못 했거나 사용자 판단이 필요한 항목은 `- [ ]` 로 남긴다 (근거 없이 전부 체크 금지)
- **`## 변경 유형` 처럼 택1 체크박스**: 정확히 하나만 `- [x]`. 두 유형 이상 해당하면 체크를 늘리지 말고 사용자에게 PR/MR 분리를 권한다
- **`Closes <이슈키>`** (이슈 키가 있을 때만): 템플릿에 이슈 슬롯(`## 관련 이슈` 등)이 있으면 그 슬롯에 넣고 **중복 append 금지**, 슬롯이 없으면 본문 마지막 줄 — 푸터 (`🤖 Generated by Padosol`) 직전. 다른 위치 금지.
  - GitHub Issues: `Closes #42` → 머지 시 이슈 자동 close
  - Linear: `Closes MP-42` → **Linear 의 GitHub/GitLab 연동이 설정된 저장소에서만** 자동 Done 전이. 연동이 없으면 아무 동작도 하지 않는 평문일 뿐이다
  - 이슈 키가 없으면 이 줄을 아예 넣지 않는다 (빈 `Closes` 나 placeholder 금지). 템플릿에 이슈 슬롯·안내 주석이 있으면 그 섹션째로 삭제
...
규칙:
- `Closes <이슈키>` 매직워드는 **이슈 키가 있을 때만** (없으면 생략이 정상)
...
- ❌ 이슈 키 placeholder (`MP-XXX`, `#0`) 로 PR 생성 — 없는 이슈를 가리키는 매직워드, 제목에도 의미 없는 문자열이 남음
  ✅ 못 찾으면 **그냥 생략**하고 진행 (`<type>: <요약>` 제목 + `Closes` 줄 없음)
- ❌ 이슈 키가 없다고 사용자에게 되물어 흐름을 끊음 — 이슈 트래커를 안 쓰는 저장소가 정상 케이스
  ✅ 자동 추출 실패 시 질문 없이 생략. 사용자가 인자로 준 경우에만 그 키 사용
- ❌ 모든 저장소가 Linear 를 쓴다고 가정 — GitHub Issues 만 쓰는 저장소에서 `Closes MP-42` 는 아무 동작도 안 하는 평문
  ✅ 브랜치·커밋에서 발견된 형식을 그대로 따름 (`MP-42` / `#42`). 트래커를 임의로 단정하지 않음
...
- ❌ Closes 매직워드를 커밋 메시지에 기재 — squash 시 소실
  ✅ PR/MR 본문에만 기재
- ❌ 프로젝트 PR/MR 템플릿이 있는데 내장 fallback 템플릿으로 덮어씀 — 팀 컨벤션 무시
  ✅ 프로젝트 템플릿 탐지되면 그 본문을 토대로 슬롯만 채움 (섹션 보존), 없을 때만 내장 fallback
- ❌ 내장 템플릿 선택을 SKILL 본문에서 임의로 결정 (예: GitLab 인데 feature.md) — 호스트 컨벤션과 어긋남
  ✅ 어떤 파일을 쓸지는 `<가이드: 내장 fallback 템플릿>` 이 정함 (GitHub=상황별 4종, GitLab=`mr.md` 고정)
- ❌ 상황 판단이 확실한데도 매번 AskUserQuestion 으로 되물음 — 하이브리드 취지 훼손, 흐름 끊김
  ✅ 브랜치 prefix 매칭 또는 커밋 type 최빈 과반이면 자동 확정, 혼재/불명확일 때만 사용자에게 확인
- ❌ 규약 체크리스트를 근거 없이 전부 `- [x]` 로 채움 — 리뷰어에게 거짓 신호
  ✅ 실제 확인한 항목만 체크, 나머지는 `- [ ]` 로 남겨 사용자 판단에 맡김. 항목 삭제·추가 금지
- ❌ 이슈 슬롯(`## 관련 이슈`)에 넣고 본문 끝에도 `Closes` 를 또 append — 중복 매직워드
  ✅ 슬롯이 있으면 슬롯에만, 없으면 마지막 줄에만
...
Claude: [/safe-commit] 으로 commit 끝남 → /open-pr 호출
        ...
        → 검증 → 이슈 키 추출: 브랜치 `refactor/MP-84-...` → ISSUE_KEY=MP-84
        → 제목 `refactor: MP-84 <요약>`
...
```
사용자: PR 올려줘
Claude: → 브랜치 `feature/duo-tab-rename` — 브랜치·커밋 어디에도 이슈 키 없음
        → ISSUE_KEY 비움. **묻지 않고** 그대로 진행
        → 제목 `feat: 듀오 찾기 탭 명칭 정리`  (대괄호 접두 없음)
        → templates/feature.md 슬롯 채움, `Closes` 줄은 넣지 않음
        → PR 생성 후 종료
...
Claude: → §0 탐지 (GF_HOST=gitlab, settings.json git_host=gitlab) → GITLAB_GUIDE.md
        → 본문 생성: `.gitlab/merge_request_templates/` 없음
          → <가이드: 내장 fallback 템플릿> = templates/mr.md 고정
          → `## 관련 이슈` 슬롯에 Closes MP-12, `## 변경 유형` 은 feat 하나만 체크,
            `## 체크리스트` 는 실제 확인한 항목만 체크
...
- `templates/feature.md` · `refactor.md` · `bugfix.md` · `general.md` — 프로젝트 템플릿이 없을 때 GitHub 이 쓰는 내장 상황별 골격 (이 스킬 디렉토리)
- `templates/mr.md` — 프로젝트 템플릿이 없을 때 GitLab 이 쓰는 단일 MR 골격 (관련 이슈 / 요약 / 변경 유형 택1 / 변경 내용 / 테스트·검증 / 체크리스트)
